export function StatusBar(props: { version?: number; selected: number; busy: boolean; message: string }) {
  return <footer className="flex h-7 items-center justify-between border-t border-white/[.07] bg-[var(--surface-1)] px-3 text-[10px] text-zinc-500">
    ...
    <div className="ml-4 flex shrink-0 items-center gap-4"><span>{props.selected ? `${props.selected} selected` : 'No selection'}</span><span>Markdown</span>{props.version ? <span>v{props.version}</span> : null}</div>
  </footer>;
}
