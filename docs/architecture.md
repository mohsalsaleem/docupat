...
main (composition root)
  ...
  ├── semantic             optional cached similarity retriever
...
- `internal/semantic` provides the optional fallback: cached embeddings, cosine ranking, score thresholds, and a maximum-result bound.
...
The context manifest is inspectable in the review dialog, persisted with the patch, and sent to the model as read-only material. Each document write incrementally replaces only that document's section and link rows in SQLite. Deterministic relationships are always resolved first. If explicitly enabled and those relationships are insufficient, semantic retrieval may add a few scored sources from cached section embeddings. A rule-based confidence assessment summarizes structural, explicit, semantic, and unresolved evidence without another model call. Incoming links, linked diagrams, and strong semantic neighbors are persisted as non-mutating impact findings for follow-up review. Generation remains a separate, final model request.
